...
export const Sidebar: React.FC<SidebarProps> = ({ items, selectedId, onSelect, onContextMenu }) => {
  ...
  return (
    <div className="sidebar w-[260px] bg-white border-r border-[#ddd]">
      <div className="p-4">
        {hardcodedItems.map((item) => (
          <div
            key={item.id}
            className={`sidebar-item flex items-center px-4 py-1 text-sm text-[#444] leading-6 cursor-pointer hover:bg-[#eef0f2] transition-colors duration-150 ${
              selectedId === item.id ? 'selected bg-[#dde1e5] border-l-3 border-l-[#007aff]' : ''
            }`}
